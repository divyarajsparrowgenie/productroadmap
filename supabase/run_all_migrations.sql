...
CREATE POLICY "Users manage milestones of own features" ON public.milestones FOR ALL
  USING (EXISTS (SELECT 1 FROM public.features WHERE features.id = milestones.feature_id AND features.user_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM public.features WHERE features.id = milestones.feature_id AND features.user_id = auth.uid()));
...
CREATE TABLE IF NOT EXISTS public.version_dependencies (
  ...
  dependency_type TEXT NOT NULL DEFAULT 'blocks' CHECK (dependency_type IN ('blocks','is_blocked_by','relates_to')),
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE POLICY "Users manage version deps" ON public.version_dependencies FOR ALL
  USING (EXISTS (SELECT 1 FROM public.versions v JOIN public.features f ON f.id = v.feature_id WHERE v.id = version_dependencies.source_id AND f.user_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM public.versions v JOIN public.features f ON f.id = v.feature_id WHERE v.id = version_dependencies.source_id AND f.user_id = auth.uid()));
